...
class _aggregate(Function):
    ...
    def forward(ctx, A, X, C):
        ...
        if A.is_cuda:
            E = ops.gpu.aggregate_forward(A, X, C)
        else:
            E = ops.cpu.aggregate_forward(A, X, C)
        return E
    ...
    def backward(ctx, gradE):
        ...
        if A.is_cuda:
            gradA, gradX, gradC = ops.gpu.aggregate_backward(gradE, A, X, C)
        else:
            gradA, gradX, gradC = ops.cpu.aggregate_backward(gradE, A, X, C)
...
class _scaled_l2(Function):
    ...
    def forward(ctx, X, C, S):
        if X.is_cuda:
            SL = ops.gpu.scaled_l2_forward(X, C, S)
        else:
            SL = ops.cpu.scaled_l2_forward(X, C, S)
    ...
    def backward(ctx, gradSL):
        ...
        if X.is_cuda:
            gradX, gradC, gradS = ops.gpu.scaled_l2_backward(gradSL, X, C, S, SL)
        else:
            gradX, gradC, gradS = ops.cpu.scaled_l2_backward(gradSL, X, C, S, SL)
